...
function find_device(xml, deviceType) {
    var device = null;

    if (xml.deviceList) {
        device = xml.deviceList[0].device.filter(function (device) {
            return device.deviceType[0] == deviceType;
        })[0];
    }
...
function find_service(xml, serviceType) {
    var service = null;
    
    if (xml.serviceList) {
        service = xml.serviceList[0].service.filter(function (service) {
            return service.serviceType[0] == serviceType;
        })[0];
    }
      ...
      var request = requests[location.href] = http.request(location, function (response) {
          ...
          response.on("end", function() {
              xml2js.parseString(resbuf, function (err, data) {
                  data = data.root;
                  var base = data.URLBase || "";

                  console.log(base);
                  var wandevice = find_device(data.device[0], WANDEVICE),
                      conndevice = find_device(wandevice, WANCONNDEVICE),
                      wanip = find_service(conndevice, WANIP);

                  // console.log("IP-ing");
                  //     wanip = find_device(wandevice, WANIP);
